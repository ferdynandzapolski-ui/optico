...
void GoMemIntrinsicPass::ensureTypes(Module &M) {
    ...
    MemsetFn = M.getOrInsertFunction("__go_memset", Type::getVoidTy(Ctx), PtrTy, Type::getInt32Ty(Ctx), SizeTy, GradeTy);
}
            ...
            for (Instruction &I : llvm::make_early_inc_range(BB)) {
                if (auto *CI = dyn_cast<CallInst>(&I)) {
                    ...
                    if (Callee->getName().starts_with("llvm.memcpy")) {
                        ...
                        Value *G = nullptr;
                        if (auto *MD = CI->getMetadata("go.grade")) {
                            G = cast<ValueAsMetadata>(cast<MDNode>(MD)->getOperand(0))->getValue();
                        }
                        if (!G) G = getTOP();

                        Builder.CreateCall(MemsetFn, {Dst, Val, Len, G});
                        CI->eraseFromParent();
                    }
                }
            }
