            ...
            for (Instruction &I : llvm::make_early_inc_range(BB)) {
                if (auto *CI = dyn_cast<CallInst>(&I)) {
                    ...
                    if (Name.starts_with("llvm.memcpy")) {
                        ...
                        Value *GDst = nullptr, *GSrc = nullptr;
                        if (auto *MDDst = CI->getMetadata("go.grade.dst")) GDst = cast<ValueAsMetadata>(cast<MDNode>(MDDst)->getOperand(0))->getValue();
                        if (auto *MDSrc = CI->getMetadata("go.grade.src")) GSrc = cast<ValueAsMetadata>(cast<MDNode>(MDSrc)->getOperand(0))->getValue();
                        if (!GDst) GDst = getTOP();
                        if (!GSrc) GSrc = getTOP();
                        ...
                        Value *Len = CI->getArgOperand(2);
                        Value *G = nullptr;
                        if (auto *MD = CI->getMetadata("go.grade.dst")) G = cast<ValueAsMetadata>(cast<MDNode>(MD)->getOperand(0))->getValue();
                        if (!G) G = getTOP();
                        Builder.CreateCall(MemsetFn, {Dst, G, Val, Len});
                        CI->eraseFromParent();
                    }
                }
            }
